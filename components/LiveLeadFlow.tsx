...
export function LiveLeadFlow() {
  ...
  return (
    <section className="relative px-5 py-14 sm:px-8 lg:px-12 lg:py-16">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 rounded-3xl border border-white/10 bg-white/[0.045] p-5 shadow-[0_26px_80px_rgba(0,0,0,0.18)] backdrop-blur sm:p-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
        <div>
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            ...
            <button
              ...
              className="min-h-11 rounded-full border border-[#E8D7B9]/35 bg-white/[0.055] px-6 py-2 text-sm font-bold text-[#F7F2E8] transition hover:border-[#E8D7B9]/60 hover:bg-white/[0.09]"
            >
              Zobacz chatbota
            </button>
            <button
              type="button"
              onClick={resetFlow}
              className="min-h-11 rounded-full border border-white/10 bg-white/[0.035] px-6 py-2 text-sm font-bold text-[#D6D3D1] transition hover:border-[#E8D7B9]/35"
            >
              Reset
            </button>
          </div>
        ...
        <div className="grid gap-4">
            ...
            {flowSteps.map((step, index) => {
              ...
              return (
                <article
                  ...
                  } ${isCurrent ? "ring-2 ring-[#E8D7B9]/35" : ""}`}
                >
                  <span
          ...
          <div className="rounded-3xl border border-[#E8D7B9]/18 bg-[#0E2A24]/70 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.18)] sm:p-5">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-bold text-[#E8D7B9]">
                {activeStep < 0 ? "Podgląd aktywnego kroku" : activePreviewStep.previewTitle}
              </p>
              <span className="rounded-full border border-[#E8D7B9]/20 bg-[#E8D7B9]/10 px-3 py-1 text-xs font-bold text-[#E8D7B9]">
                live demo
              </span>
            </div>
            ...
            {activeStep < 0 ? (
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                {activePreviewStep.fields.map((field) => (
                  <span key={field} className="rounded-2xl border border-white/10 bg-white/[0.06] px-3 py-2 text-sm text-[#F7F2E8]">
                    {field}
                  </span>
                ))}
              </div>
            ) : activePreviewStep.row ? (
              <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-[#F7F2E8] text-[#171717]">
                <div className="grid grid-cols-4 bg-[#0F8A6C]/12 px-3 py-2 text-xs font-bold text-[#0E2A24]">
                  <span>Data</span>
                  <span>Imię</span>
                  <span>Branża</span>
                  <span>Status</span>
              ...
              <p className="mt-4 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm leading-6 text-[#F7F2E8]">
