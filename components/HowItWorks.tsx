      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {steps.map((step, index) => (
            <article
              key={step.title}
              className="rounded-3xl border border-[#E8D7B9]/14 bg-white/[0.055] p-6 shadow-xl shadow-emerald-950/20"
            >
              <span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-[#E8D7B9]/25 bg-[#0F8A6C]/12 text-sm font-bold text-[#E8D7B9]">
                0{index + 1}
              </span>
        ...
        <div className="mt-10 rounded-3xl border border-[#E8D7B9]/14 bg-[#171717]/65 p-5 shadow-2xl shadow-emerald-950/20">
          <div className="grid gap-4 md:grid-cols-4">
            {flow.map((item, index) => (
              <div key={item} className="relative">
                <div className="rounded-2xl border border-[#E8D7B9]/18 bg-[#E8D7B9]/[0.06] p-5 text-center text-sm font-semibold text-[#F7F2E8]">
                  {item}
                </div>
                {index < flow.length - 1 ? (
                  <div className="absolute right-[-1.25rem] top-1/2 hidden -translate-y-1/2 text-[#E8D7B9] md:block">
