...
export function ChatModal({ suggestions }: ChatModalProps) {
  ...
  return (
    <>
      ...
      {isOpen ? (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[#171717]/78 px-3 py-4 backdrop-blur-xl sm:px-6"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div className="animate-modal-in flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-[#E8D7B9]/14 bg-[#171717] shadow-2xl shadow-emerald-950/36">
            <div className="flex items-start justify-between gap-4 border-b border-white/10 bg-white/[0.06] px-5 py-4 sm:px-6">
              ...
              <button
                ...
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.06] text-lg font-semibold text-white transition hover:border-[#E8D7B9]/40 hover:bg-white/[0.1]"
                aria-label="Zamknij demo chatu"
              >
                X
              </button>
            </div>
            <div className="min-h-0 overflow-y-auto bg-[#171717] p-3 sm:p-4">
